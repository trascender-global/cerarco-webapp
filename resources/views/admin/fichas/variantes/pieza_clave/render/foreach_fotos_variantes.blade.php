<div id="fotos_variantes_draggable_{{$variante}}" class="row"
    style="display: flex">
    @foreach ($modelo->fotosVariante()->variante($variante)->codigo($varianteCodigo)->orderBy('posicion')->get() as $foto)
        <div id="variante_foto_content_{{ $foto->id }}" data-id="{{ $foto->id }}"
            style="margin: 5px;width: 90px;position: relative;padding: 0px 10px;text-align: center;background: white;box-shadow: 1px 1px 15px -5px black;border-radius: 10px;"
            data-posicion="{{ $foto->posicion }}" class="variante_foto_content_{{ $variante }}">
            <img class="img move" width="70" height="70" src="{{ $foto->foto }}" data-holder-rendered="true" />
            <button type="button" style="position: absolute;right: -5px;"
                class="btn btn-light p-0 avatar-xs rounded-circle alert-danger borrar_foto_variante"
                data-foto="{{ $foto->id }}">
                <span class="avatar-title  bg-transparent text-reset"><i class="bx bx-trash-alt"></i></span>
            </button>
        </div>
    @endforeach
</div>
<div style="width: 85px;text-align: center;margin: 5px;">
    <input style="display: none" type="file" accept="image/*" data-modelo="{{ $modelo->id }}"
        data-variante="{{ $variante }}" data-codigo="{{ $varianteCodigo }}" multiple
        id="variante_foto_input_{{ $variante }}" class="form-control input_foto_variante">
    <img width="70" height="70" style="opacity: 0.6;cursor: pointer;" class="btn_add_foto_variante"
        data-id="{{ $variante }}"
        src="data:image/png;base64," />
</div>
<script>
    function updateOrdenVariantes{{$variante}}() {
        var orden = [];
        $("#fotos_variantes_draggable_{{$variante}} div").each(function(index, element) {
            if ($(this).data('id')) {
                orden.push({
                    "id": $(this).data('id'),
                    'index': index
                })
                
            }
        });
        $.ajax({
            type: 'POST',
            url: "{{ route('admin.ficha.mover_fotos_variante', compact('modelo', 'variante', 'varianteCodigo')) }}",
            data: {
                '_token': $('input[name=_token]').val(),
                'orden': orden
            },
            success: function(data) {},
            error: function(xhr, status) {},
        });
    }
    $("#fotos_variantes_draggable_{{ $variante }}").sortable({
        handle: '.move',
        update: function(event, ui) {
            updateOrdenVariantes{{$variante}}()
        }
    });
    $("#fotos_variantes_draggable_{{ $variante }}").disableSelection();
</script>
