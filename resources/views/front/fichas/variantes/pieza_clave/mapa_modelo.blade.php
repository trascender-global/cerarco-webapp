...
<div id="mapa_modelo" style="width: auto; height: 400px;border-radius: 15px"></div>

<div class="container-fluid p-3">
    <!-- Range slider: -->
    <input id="buscador_cronologia_modelo" type="text"
        ...
        data-slider-step="100" data-slider-ticks-snap-bounds="200" data-slider-ticks-tooltip="true"
        data-slider-handle="custom" data-slider-disable="true"
...
<script>
    ...
    var map_modelo = new mapboxgl.Map({
        container: 'mapa_modelo',
        style: 'mapbox://styles/mapbox/streets-v11',
        center: [-74.04, 4.107],
        zoom: 4
    });

    var currentMarkers_modelo = [];
    @foreach ($modelo->piezasClave as $pieza_clave)
        @foreach ($pieza_clave->puntos as $punto)
            var marker = new mapboxgl.Marker({
                ...
                .addTo(map_modelo);
            currentMarkers_modelo.push(marker);
        @endforeach
    @endforeach


    var slider_modelo = new Slider("#buscador_cronologia_modelo", {
        ticks: [0, 500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4100, 4200, 4300, 4400, 4500, 4600, 4700,
        ...
        ticks_positions: [0, 6, 13, 19, 26, 32, 38, 45, 51, 53, 54, 55, 56, 58, 59, 60, 62, 63, 64, 65, 67, 68,
            69, 71, 72, 73, 74, 76, 77, 78, 79, 81, 82, 83, 85, 86, 87, 88, 90, 91, 92, 94, 95, 96, 97, 99,
            100
        ],
        ticks_snap_bounds: 200,
        ticks_labels: ['6000 a.C', '', '', '', '', '', '', '', '2000 a.C', '', '', '', '', '', '', '', '', '',
            '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
            '', '', '', '1800 d.C.'
        ],
        formatter: function(a) {
            if (Array.isArray(a)) {
                let r = a[0] - 6e3,
                    e = a[1] - 6e3;
                return 0 > r ? r = Math.abs(r) + " a.C" : r += " d.C", 0 > e ? e = Math.abs(e) + " a.C" :
                    e += " d.C", r + " a " + e
            }
        ...
        ticks_tooltip: true,
        step: 100,
        value: [{{ calcular_cronologia_minimo($pieza_clave->datos_cronologia) }},
            {{ calcular_cronologia_maximo($pieza_clave->datos_cronologia) }}
        ],
    });
    slider_modelo.disable();
</script>
